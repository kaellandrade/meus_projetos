  ...
  return (
    <View
      style={[
        estilos.boxFrind,
        isDark
          ? {borderColor: '#ababab', backgroundColor: '#0000'}
      ...
      <Box
        style={[
          ...
          isDark ? {borderColor: '#fffa'} : {borderBottomColor: 'black'},
        ]}>
      ...
      <View style={{flex: 1}}>
        <Text
          style={[estilos.nome, isDark ? {color: '#fffa'} : {color: 'black'}]}>
          {item.name}
        </Text>
        <Text
          style={[estilos.email, isDark ? {color: '#fff8'} : {color: 'black'}]}>
          {item.email}
        </Text>
      </View>
      ...
      <View style={estilos.btnGroups}>
        <IconBtn
          color={
            isDark
              ? ESTILOS_COMUNS.cores.secundaria
              : ESTILOS_COMUNS.cores.principal
          }
          Onpress={_ =>
            props.openModal({name: item.name, email: item.email, id: item.id})
          }
          name="edit"
        ...
        <IconBtn
          ...
          Onpress={_ => props.deleteFriend(item.id)}
          name="trash"
          size={ESTILOS_COMUNS.iconesTamanhos.grande}
...
const AmigoSecreto = props => {
  ...
  return props.telaSorteando ? (
    <SorteioSpinner texto="Sorteando amigos" />
  ) : (
    <View
      style={[
        estilos.container,
      ...
      <SafeAreaView style={estilos.conteudo}>
        {amigosCadastrados.length === VAZIO ? (
          <TutorialAdd />
        ) : (
          <FlatList
        ...
        <TouchableWithoutFeedback
          ...
          <View
            style={[
              estilos.butonAdd,
              isDark ? {borderColor: '#ffff'} : {borderColor: '#444444'},
            ]}>
            <Icon
